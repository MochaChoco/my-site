...
onMounted(() => {
  ...
  const { callbackUrl, redirectUri, serviceId, clientId, responseType, scope, state } = route.query
  signupStore.setCallbackUrl(callbackUrl || redirectUri || '')
  signupStore.setServiceId(serviceId ?? '')
  signupStore.setClientId(clientId ?? '')
  signupStore.setResponseType(responseType ?? '')
  signupStore.setScope(scope ?? '')
  signupStore.setState(state ?? '')
})
</script>
```
...
const onClickHandler = () => {
  ...
  if (isWebView) {
    ...
    let url = '/social-login/google'
    if (callbackUrl && serviceId) {
      url += `?callbackUrl=${callbackUrl}&serviceId=${serviceId}`
    }
    window?.AppBridge?.startSocialLogin(baseUrl + url)
    return
  }
  ...
  try {
    ...
    script.onerror = () => showError('인증 스크립트 로드에 실패했습니다.')
...
function handleNaverLogin() {
  ...
  const naverAuth = SocialAuthService.Naver.init('LOGIN', buildQueryString(route.query))
...
SocialAuthService.Naver.init = (mode, queryParams) => {
  const callbackUrl =
    window.location.origin + '/callback/social/naver?' + buildNaverCallbackQuery(mode, queryParams)
...
<template>
  <div class="email-login-page">
    <form v-on:submit.prevent="submitForm">
      <AppLogo :width="logoSize.width" :height="logoSize.height" />
...
const formFields = ref({
  ...
  password: {
    ...
    error: null,
    validate: (value) => {
      return value.length >= 6 ? null : '비밀번호는 6자 이상이어야 합니다.'
    }
  }
})

const isFormValid = computed(() => {
  return formFields.value.email.value && formFields.value.password.value
})
...
async function onSubmit() {
  try {
    // authStore.login() 호출: POST /api/v1/auth/login
    ...
    formFields.value.email.error = true
    formFields.value.password.error = true
    // 에러 토스트 표시
...
function getErrorMessage(err) {
  ...
  return '일시적인 오류로 로그인을 할 수 없습니다. 잠시 후에 다시 이용해주세요.'
}
</script>
```
...
import EmailVerificationStep from '@/components/signup/EmailVerificationStep.vue'
import AgeVerificationStep from '@/components/signup/AgeVerificationStep.vue'
import CompletionStep from '@/components/signup/CompletionStep.vue'
import { useSignupStore } from '@/stores/signup'
import { storeToRefs } from 'pinia'
import { useIdentityVerification } from '@/composables/useIdentityVerification'

const signupStore = useSignupStore()
const { getVerificationInfo, getRedirectUrl } = useIdentityVerification()
...
onBeforeMount(() => {
  ...
  const savedBirthday = signupStore.getBirthday()

  if (
    (verificationInfo && redirectUrl) ||
    ((!verificationInfo || !redirectUrl) && savedBirthday)
  ) {
...
export function useSocialAuth() {
  ...
  async function handleAuthFailure(provider, token) {
    ...
    const { email, exists, emailExists, emailVerified, nickname, birthday } = data

    // Signup Store에 소셜 정보 프리필
    signupStore.setSocialProvider(provider)
    signupStore.setSocialToken(token)
    if (nickname) signupStore.setNickname(nickname.substring(0, 30))
    if (birthday) signupStore.setBirthday(birthday)

    // 에러 케이스별 처리
    if (exists) {
      showError('이미 가입한 이메일입니다. 이메일·비밀번호로 로그인하시거나, 다른 이메일로 가입해주세요.')
      return
    } else if (emailExists) {
      showError(`해당 "${email}"는 이메일로 가입된 계정이에요. 이메일 계정으로 로그인하신 후, 소셜 계정을 연동해 주세요.`)
      return
    } else if (emailVerified === false) {
      showError('이메일 인증이 완료되지 않았습니다. 해당 소셜 사이트에서 이메일 인증 후 진행해주세요.')
      return
    }

    // 신규 회원 → 회원가입 페이지로 이동 (이메일 자동 입력)
    signupStore.setEmail(email)
    router.push('/signup')
  }

  async function linkAccount(provider, token, isSignup) {
    try {
      await authStore.linkSocialAccount(provider, token)
      console.info(`Successfully linked ${provider}`)
      ...
      showError('계정 연동에 실패했습니다.')
    }
  }

  async function unlinkAccount(provider) {
    try {
      await authStore.unlinkSocialAccount(provider)
      console.info(`Successfully unlinked ${provider}`)
    } catch (error) {
      console.error(`Error unlinking ${provider}`, error)
      showError('계정 연동 해제에 실패했습니다.')
    }
  }

  return { authenticate, linkAccount, unlinkAccount }
}
```
...
export function usePostAuth() {
  ...
  let callbackUrl = route.query?.callbackUrl
  let responseType = route.query?.response_type
  let clientId = route.query?.client_id
  ...
  // 서비스 리다이렉트 여부
  function isServiceRedirect() {
    return serviceId && callbackUrl
  }

  // OAuth 클라이언트 승인
  async function approveOAuthRequest() {
    const { data } = await authApi.approveOAuth(
      clientId, responseType, redirectUri, scope, state
    )

    let url
    if (responseType === 'code') {
      url = redirectUri + '?code=' + data.code
      if (state) url += '&state=' + state
    } else {
      url = redirectUri + '#accessToken=' + data.accessToken
      if (state) url += '&state=' + state
    }
  ...
  async function redirectToService() {
    const service = await serviceStore.getService(serviceId)

    if (!service) {
      showError('올바른 서비스 명이 아닙니다.')
      return
    }

    window.location.href =
      window.location.origin +
      '/api/v1/auth/complete?serviceId=' + serviceId +
      '&callbackUrl=' + callbackUrl
  }
...
export const useSignupStore = defineStore('signup', () => {
  ...
  function redirectToCallback() {
    ...
    } else if (clientId.value && responseType.value) {
      query.client_id = clientId.value
      query.response_type = responseType.value
      query.redirect_uri = redirectUri.value
    ...
    strategies: [
      {
        ...
        paths: ['email', 'nickname', 'birthday', 'socialProvider', 'currentStep']
      }
    ]
  }
})
...
`usePostAuth` 컴포저블 하나로 이메일 로그인, 네이버 로그인, 구글 로그인, 회원가입 모두 동일한 후처리 로직을 공유합니다. 새로운 소셜 로그인 제공자를 추가하더라도 해당 인증 처리 로직만 구현하면 됩니다.

### 유지보수성

리다이렉트 로직을 수정해야 할 때, 하나의 파일만 수정하면 모든 로그인 방식에 즉시 반영됩니다.

### 일관된 사용자 경험

어떤 방식으로 로그인하든 동일한 흐름으로 최종 목적지(메인 페이지 또는 외부 서비스)로 이동하므로, 사용자는 일관된 경험을 받습니다.

### 확장성

OAuth 2.0 표준을 준수하여 외부 서비스 연동이 쉽고, 생태계 내 여러 서비스와의 SSO(Single Sign-On)도 자연스럽게 지원됩니다.

## 마무리

처음에는 로그인 버튼 3개만 만들면 해결될 것이라고 생각했습니다.
하지만 구현을 진행하면서 OAuth 콜백 처리, 신규/기존 회원 분기, 외부 서비스 리다이렉트, 실패 처리 등 고려해야 할 케이스가 예상보다 훨씬 많았습니다.

이에 전체 플로우를 다이어그램으로 시각화해 정리했고,
그 결과 공통 후처리 레이어(usePostAuth)를 설계·구현하는 과정도 한층 수월해졌습니다.
또한 이후 디버깅과 유지보수 측면에서도 흐름을 빠르게 추적할 수 있어 작업 효율이 크게 개선되었습니다.
...
GET 리다이렉트 방식은 URL에 결제 식별자를 포함하여 PG 결제창으로 이동하는 방식입니다. PG사 A는 서버에서 미리 결제 정보를 등록하고 발급받은 `reserveId`를 URL 경로에 포함하여 결제창을 호출합니다.
...
[클라이언트] ←─────────────────────────┘
      ↓
window.location.href = "https://pg-a.example.com/payments/{reserveId}"
      ↓
[PG사 A 결제창] → 결제 진행
      ↓
GET 콜백: /payment/callback?resultCode=Success&orderId=xxx
```
...
- URL에 민감 정보 노출 위험 (따라서 reserveId 방식 사용)
...
PG사 A는 `reserveId` 기반의 GET 리다이렉트를 사용합니다. 이 방식의 핵심은 결제 정보가 URL에 직접 노출되지 않고, 서버에서 미리 등록한 결제 정보를 식별하는 ID만 전달된다는 점입니다.

```tsx
function openEasyPayA(paymentData: PaymentDataType) {
  const { reserveId } = paymentData;
  ...
  // reserveId를 URL 경로에 포함하여 결제창 호출
  let paymentUrl = `${pgDomain}/payments/${reserveId}`;
...
`reserveId` 방식의 장점은 **위변조 방지**입니다. 결제 금액, 상품 정보 등이 URL에 노출되지 않고, 서버에서 미리 검증된 정보로만 결제가 진행됩니다.
...
export default function PaymentSubmitForm() {
  ...
  function validateAndSetParams() {
    switch (payMethod) {
      ...
      case "EASY_PAY_B":
        // 간편결제 β: 트랜잭션ID
        if (!pgUrl || !txId) return false;
        setParamName("transactionId");
        setParamValue(txId);
        break;
...
export async function GET(request: NextRequest) {
  ...
  if (resultCode !== "Success") {
    return redirect(
      `/payment/fail?orderId=${orderId}&resultCode=UserCancel`,
      RedirectType.push
    );
  }
  ...
  // 통합 완료 페이지로 리다이렉트
  return redirect(
    `/payment/complete?payMethod=EASY_PAY_A&orderId=${orderId}&returnParams=${encodeURIComponent(
      returnParams
    )}`,
    RedirectType.push
  );
...
export async function POST(request: NextRequest) {
  ...
  // 통합 완료 페이지로 리다이렉트
  return redirect(
    `/payment/complete?payMethod=${methodType}&orderId=${orderId}&returnParams=${
...
export async function POST(request: Request) {
  ...
  const { orderId, payMethod, userPlatform } = body;

  // 백엔드 API 호출
  const result = await fetchBackendAPI(`/api/v1/payments/${orderId}/prepare`, {
    method: "POST",
    body: JSON.stringify({
      payMethod,
      userPlatform,
...
4. **보안 강화**:
   - PG사 A: reserveId 방식으로 결제 정보 위변조 방지
   - PG사 B: 암호화된 파라미터 사용
